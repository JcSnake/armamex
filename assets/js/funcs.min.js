...
function validateRemolqueForm() {
    ...
    var frmTripulacion = $('#cotOpt2Condicion')[0];
...
function sendContactForm() {
    var frm = $('#contactFrm')[0];
    var data = '&frmName=' + frm.frmName.value + '&frmEmail=' + frm.frmEmail.value + '&frmPhone=' + frm.frmPhone.value + '&frmMessage=' + frm.frmMessage.value;
	...
	$.ajax({
		...
		success: function(response) {
			if(response == 1) {                
				...
				mensaje = 'No se pudo enviar tu mensaje por este medio, le recomendamos intentar de nuevo o enviar un correo directo a <a href="mailto:contacto@maspm.lat?subject=Error en el formulario">contacto@maspm.lat</a>.<br><br>Gracias por contactarnos.';
                showNotification(mensaje, NOTIFY_STS.FAIL, NOTIFY_POS.TOP_CENTER);
		...
		error: function(xhr, status, error) {
            ...
            var mensaje = 'Se genero un error al tratar de enviar sus comentarios, le recomendamos intentar de nuevo o enviar un correo directo a <a href="mailto:contacto@maspm.lat?subject=Error en el formulario">contacto@maspm.lat</a>.<br><br>Gracias por contactarnos.';
			showNotification(mensaje, NOTIFY_STS.FAIL, NOTIFY_POS.TOP_CENTER);
		}
	});
}
...
function sendFletamentosForm() {
    var frm = $('#solicitanteFrm')[0];
    var frm1 = $('#cotizacionOpt1Frm')[0];
    var data = '&cotizacionName=' + frm.cotizacionName.value + '&cotizacionEmail=' + frm.cotizacionEmail.value + '&cotizacionService=' + frm.cotizacionService.value + '&cotizacionPhone=' + frm.cotizacionPhone.value + '&cotizacionFletador=' + frm.cotizacionFletador.value + '&cotOpt1Quantity=' + frm1.cotOpt1Quantity.value  + '&cotOpt1Type=' + frm1.cotOpt1Type.value + '&cotOpt1Capacity=' + frm1.cotOpt1Capacity.value + '&cotOpt1Area=' + frm1.cotOpt1Area.value + '&cotOpt1Entrega=' + frm1.cotOpt1Entrega.value + '&cotOpt1Principal=' + frm1.cotOpt1Principal.value + '&cotOpt1Extension=' + frm1.cotOpt1Extension.value;

	$.ajax({
		timeout: 25000,
		type: 'POST',
		url: 'app/ws/sendEmailCotizacionFletamentos.php',
		data: data,
		beforeSend: function() {
		},
		success: function(response) {
...
function sendRemolquesForm() {
    var frm = $('#solicitanteFrm')[0];
    var frm1 = $('#cotizacionOpt2Frm')[0];
    var data = '&cotizacionName=' + frm.cotizacionName.value + '&cotizacionEmail=' + frm.cotizacionEmail.value + '&cotizacionService=' + frm.cotizacionService.value + '&cotizacionPhone=' + frm.cotizacionPhone.value + '&cotizacionFletador=' + frm.cotizacionFletador.value + '&cotOpt2Salida=' + frm1.cotOpt2Salida.value + '&cotOpt2Peso=' + frm1.cotOpt2Peso.value + '&cotOpt2Eslora=' + frm1.cotOpt2Eslora.value + '&cotOpt2Calado=' + frm1.cotOpt2Calado.value + '&cotOpt2TRB=' + frm1.cotOpt2TRB.value + '&cotOpt2Entrada=' + frm1.cotOpt2Entrada.value + '&cotOpt2Desplazamiento=' + frm1.cotOpt2Desplazamiento.value  + '&cotOpt2Condicion=' + frm1.cotOpt2Condicion.value;

	$.ajax({
		timeout: 25000,
		type: 'POST',
		url: 'app/ws/sendEmailCotizacionRemolques.php',
		data: data,
		beforeSend: function() {
		},
		success: function(response) {
